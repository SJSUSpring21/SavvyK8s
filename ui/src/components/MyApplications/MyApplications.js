...
class MyApplications extends Component {
  ...
  getAllApps = () => {
    ...
    axios
      .get(config.backEndURL + "/users/allAppPodDtls")
      .then((response) => {
  ...
  appSelected = (e, app) => {
    ...
    const appPodIndex = appPodDtls.findIndex(
      (appPod) => appPod.appId === app.appId
    );
    if (appPodIndex > -1) {
  ...
  updateApplications = () => {
    ...
    if (this.state.appPodDtls.length > 0) {
      ...
      axios
        .put(config.backEndURL + "/users/custApp", custAppReq)
        .then((response) => {
          console.log("Status Code : ", response.status);
          if (response.status === 200) {
            console.log(response);
    ...
    } else alert("Please select atleast one application to update");
  };
  render() {
        ...
        if (checked)
          return (
            <span>
              ...
              {/* <input type="checkbox" checked={checked} value={app.appId} onChange={(e) => this.appSelected(e, app)} /> &nbsp;{app.appName}  */}
              <br />
            </span>
          );
        else
    ...
    return (
      <div className="myApplications">
        ...
        {checkedApps}
        <br />
        <button className="saveBtn" onClick={this.updateApplications}>
          Save
        </button>
      </div>
    );
