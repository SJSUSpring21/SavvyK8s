...
class AppRegistration extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(config.backEndURL + "/users/appRegistration")
      .then((response) => {
  ...
  appSelected(e, app) {
    ...
    if (e.target.checked) {
      ...
      const appIndex = appRegistration.appId.findIndex(
        (reg) => reg === app._id
      );
      if (appIndex > -1) {
  ...
  saveCustAppDtls = () => {
    ...
    if (appRegReq.appId.length > 0) {
      axios
        .post(config.backEndURL + "/users/custAppDtls", appRegReq)
        .then((response) => {
          console.log("Status Code : ", response.status);
    ...
    return (
      <div className="appregistration-root">
        <AppBar position="static">
          <Toolbar>
            <Typography variant="h3">SavvyK8s</Typography>
          </Toolbar>
        </AppBar>
        <Grid container style={{ marginTop: 25, marginLeft: 350 }} spacing={3}>
          <Grid item xs={12}>
